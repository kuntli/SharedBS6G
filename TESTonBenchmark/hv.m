function [Score,PopObj] = hv(PopObj,ReferencePoint)
% <metric> <max>
% Hypervolume
...
%--------------------------------------------------------------------------
    ...
    fmax   = ReferencePoint;
    PopObj = (PopObj-repmat(fmin,N,1));
    PopObj_N = PopObj./repmat((fmax-fmin)*1.1,N,1);
    PopObj(any(PopObj_N>1,2),:) = [];
    % The reference point is set to (1,1,...)
    RefPoint = (ReferencePoint-fmin)*1.1;
    if isempty(PopObj)
        Score = 0;
    elseif M < 4
