...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopDec = Init(obj,N,type)
            switch type
                case 'upper'
                    switch obj.Global.encoding
                        case 'binary'
                            PopDec = randi([0,1],N,obj.Global.u_D);
                        case 'permutation'
                            [~,PopDec] = sort(rand(N,obj.Global.u_D),2);
                        otherwise
                            PopDec = unifrnd(repmat(obj.Global.u_lower,N,1),repmat(obj.Global.u_upper,N,1));
                    end
                case 'lower'
                    switch obj.Global.encoding
                        case 'binary'
                            PopDec = randi([0,1],N,obj.Global.l_D);
                        case 'permutation'
                            [~,PopDec] = sort(rand(N,obj.Global.l_D),2);
                        otherwise
                            PopDec = unifrnd(repmat(obj.Global.l_lower,N,1),repmat(obj.Global.l_upper,N,1));
                    end
            end
        end
