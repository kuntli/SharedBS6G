...
% %             cgan = cgan0;
%          else
%             cgan = cgan.train(TrainSet.udecs,TrainSet.ldecs,50); 
%          end

        %% Generate the upper level Offspring
        
        [SQ,RQs] = Reproduce_BSP(N,tmax,u_Pop,Idx_ul,l_Pop,Arch_partion,W,Global);
...
%             end 
        ...
        [output,Idx_oA,Archive,RPs] = RefineSearch_BSP(output,Idx_oA,Archive,RPs,tm,Global);
...
% 
%             tmax = min(100,max(floor(tmax + tmax/5*min(5,log2(Ir1/Ir2))),1));
% 
%          end
        if gen > 10    
