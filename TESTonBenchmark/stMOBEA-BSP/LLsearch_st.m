function [SP,RP] = LLsearch_st(xu,Pt,Global)
...
%         RP =[xu,INDIVIDUAL(repmat(xu(1).udec,size(Xls,1),1),Xls,'lower')];
        ...
        Xls = rand(Nl-length(xu),l_D).*(l_upper-l_lower)+l_lower;
        RP =[xu,INDIVIDUAL(repmat(xu(1).udec,size(Xls,1),1),Xls,'lower')];
...
%         RP = INDIVIDUAL(repmat(xu,Nl,1),Xl,'lower'); %ReferencePoints
        ...
        Xl = rand(Nl,l_D).*(l_upper-l_lower)+l_lower;
        RP = INDIVIDUAL(repmat(xu,Nl,1),Xl,'lower'); %ReferencePoints
    end
    Xu = RP.udecs;
    xu = Xu(1,:);
...
%         site = max(xu-Ptu,0)>0;
%         Site = repmat(site,Nl,l_D/u_D);
%         Xl = round(rand(Nl,l_D).*(l_upper-l_lower)+l_lower);
%         Ptl(Site) = Xl(Site);

%         Ptl = reshape(Ptl,Nl,u_D,l_D/u_D).*xu; 
%         Ptl = reshape(Ptl,Nl,l_D);
        SQ = INDIVIDUAL(repmat(xu,length(Pt),1),Ptl,'lower');
    else
        SQ = [];
    end
    SR = [RP,SQ];
...
%             end
            ...
            [u_FrontNo,u_MaxFNo] = NDSort(Candidate.uobjs,Candidate.ucons,Nl);
